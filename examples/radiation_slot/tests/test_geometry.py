...
def test_dataclass_defaults():
    ...
    assert (g.waveguide_width, g.waveguide_height) == (4.0, 1.5)
...
def test_csg_inside_outside():
    ...
    assert geom.inside(np.array([[2.0, 0.5]]))[0]                                   # waveguide
    assert geom.inside(np.array([[2.0, g.waveguide_height + 0.5]]))[0]              # buffer above slot
    assert not geom.inside(np.array([[2.0, g.waveguide_height + g.buffer_height + 0.1]]))[0]   # above buffer
    assert not geom.inside(np.array([[0.2, g.waveguide_height + 0.1]]))[0]          # above wg, NOT under slot
...
def test_fdfd_grid_shape_and_mask():
    ...
    # Allow ~5% tolerance: discrete cells include slot edges.
